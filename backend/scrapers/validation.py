...
def is_scrapable(url: str, platform: Platform, page: Page = None) -> bool:
        ...
        try:
            ...
            if platform == Platform.LONGSHANKS:
                ...
                try:
                    ...
                    logger.warning(f"Validation Failed: Rollbetter check error: {e}")
                    return False
            else:
                logger.warning(f"Unsupported platform for validation: {platform}")
                return False
        except Exception as e:
             logger.warning(f"Validation Failed: Error during check for {url}: {e}")
             return False
    ...
    if page:
        ...
        try:
            ...
            logger.warning(f"Validation Failed: Browser init error for {url}: {e}")
            return False
...
def _validate_longshanks(page: Page) -> bool:
    ...
    if not is_valid_system:
        logger.warning("Validation Failed: Detected other game system or X-Wing not found.")
        return False
    ...
    if player_count < 2:
        logger.warning(f"Validation Failed: Insufficient players ({player_count})")
        return False
    ...
    if not has_games_tab and not has_ranking_rows:
        logger.warning("Validation Failed: No Games tab or Ranking rows found (Event likely unstarted).")
        return False
...
def _validate_rollbetter(page: Page) -> bool:
    ...
    if "Marvel Crisis Protocol" in body_text and not is_xwing_title:
        logger.warning("Validation Failed: Marvel Crisis Protocol detected.")
        return False
    if "Star Wars: Legion" in body_text and not is_xwing_title:
        logger.warning("Validation Failed: Legion detected.")
        return False
    if "X-Wing" not in body_text and "Miniatures Game" not in body_text and not is_xwing_title:
        logger.warning("Validation Failed: X-Wing not found in text/title.")
        return False
    ...
    if not has_rounds:
        logger.warning("Validation Failed: No Rounds tab found.")
        return False
