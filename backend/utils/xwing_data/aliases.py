...
class ShipSplitAlias:
    ...
    The two display labels are part of the entry because the vendored data
    gives both chassis the same human name; the split is only legible to a
    reader once each one carries its own label.
    """
...
SHIP_SPLIT_ALIASES: tuple[ShipSplitAlias, ...] = (
    ShipSplitAlias(
        ...
        deprecated_label="Y-wing",
        variant_label="Y-wing (Wartime Loadout)",
    ),
)

# NOTE: the frontend mirrors these labels in
# ``frontend/src/lib/data/shipLabels.json`` because the prebuilt manifest is
# generated by a Node script that cannot import this Python module. Keep both
# maps in sync when adding a split entry.
...
@lru_cache(maxsize=1)
def ship_label_overrides() -> dict[str, str]:
    """Human-facing ship name per xws for split chassis.

    The vendored data uses one shared `name` ("BTA-NR2 Y-Wing") for both
    chassis. This map is applied where the human name is derived, never to the
    ids themselves.
    """
    labels: dict[str, str] = {}
    for alias in SHIP_SPLIT_ALIASES:
        labels[alias.deprecated_id] = alias.deprecated_label
        labels[alias.variant_id] = alias.variant_label
    return labels


def ship_display_name(xws: str, fallback: str) -> str:
    """Return the human-facing label for a ship xws, else ``fallback``."""
    return ship_label_overrides().get(xws) or fallback
