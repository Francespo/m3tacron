...
def enrich_list_data(stats: dict, source: DataSource = DataSource.XWA) -> ListData:
    ...
    for p in pilots:
        pid = p.get("id") or p.get("name") or p.get("xws")
        ...
        try:
            pilot_points = int(pilot_points_raw)
        except (ValueError, TypeError):
            pilot_points = 0

        try:
            pilot_loadout = int(pilot_loadout_raw)
        except (ValueError, TypeError):
            pilot_loadout = 0
    ...
    try:
        count = int(stats.get("popularity", 0))
    except (ValueError, TypeError):
        count = 0
    ...
    return ListData(
        signature=stats.get(
            "signature", "Unknown Signature") or "Unknown Signature",
