    ...
    <div class="grid grid-cols-1 gap-2">
        <div class="flex items-center gap-1.5">
            ...
            <div class="relative flex-1">
                <input bind:this={startRef} type="date" class="w-full bg-black border border-border-dark rounded px-2 py-1.5 pr-8 text-xs font-mono text-primary focus:border-primary focus:outline-none" bind:value={startDate} aria-label="Start date" />
            ...
            {#if startDate}
                <button type="button" class="w-6 h-6 grid place-items-center rounded hover:bg-white/10 text-secondary hover:text-primary" onclick={() => (startDate = '')} aria-label="Clear start date"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18"/><path d="M6 6 12 12"/></svg></button>
            {/if}
        </div>
        <div class="flex items-center gap-1.5">
            <span class="text-[11px] font-mono text-secondary w-8 shrink-0">To</span>
            <div class="relative flex-1">
                <input bind:this={endRef} type="date" class="w-full bg-black border border-border-dark rounded px-2 py-1.5 pr-8 text-xs font-mono text-primary focus:border-primary focus:outline-none" bind:value={endDate} aria-label="End date" />
            ...
            {#if endDate}
                <button type="button" class="w-6 h-6 grid place-items-center rounded hover:bg-white/10 text-secondary hover:text-primary" onclick={() => (endDate = '')} aria-label="Clear end date"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18"/><path d="M6 6 12 12"/></svg></button>
            {/if}
        </div>
    </div>
</div>
