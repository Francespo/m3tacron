<script lang="ts">
  ...
  let availableCountries = $derived(() => {
    ...
    for (const c of conts) {
      if (locationHierarchy[c]) {
        Object.keys(locationHierarchy[c]).forEach((co) => countries.add(co));
      }
    }
    ...
    for (const c of conts) {
      ...
      let countryKeys =
        filters.selectedCountries.length > 0
          ? filters.selectedCountries.filter((cO) => locationHierarchy[c][cO])
  ...
  function toggleCity(c: string) {
    if (filters.selectedCities.includes(c)) {
      filters.selectedCities = filters.selectedCities.filter((x) => x !== c);
    } else {
...
<div class="w-full space-y-3">
  ...
  <div class="border-b border-border-dark">
    ...
    >
      <span class="text-xs font-mono font-bold tracking-wider">Date Range</span>
      <svg
  ...
  <div class="border-b border-border-dark">
    ...
    >
      <span class="text-xs font-mono font-bold tracking-wider">Location</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="14"
        height="14"
            ...
            <div class="max-h-[100px] overflow-y-auto space-y-1">
              {#each filteredContinents as c}
                ...
                >
                  ...
                  />
                  <span class="font-mono truncate" title={c}>{c}</span>
                </label>
              {/each}
            </div>
          </div>
              ...
              {#each filteredCountries as c}
                ...
                >
                  <input
                    type="checkbox"
                    class="rounded border-border-dark bg-black w-4 h-4"
                    checked={filters.selectedCountries.includes(c)}
  ...
  <div class="border-b border-border-dark">
    ...
    >
      <span class="text-xs font-mono font-bold tracking-wider">Format</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="14"
        height="14"
            ...
            {#each group.formats as f}
              ...
              >
                <input
                  type="checkbox"
                  class="rounded border-border-dark bg-black w-4 h-4"
                  checked={filters.selectedFormats.includes(f.id)}
  ...
  <div class="border-b border-border-dark">
    ...
    >
      <span class="text-xs font-mono font-bold tracking-wider">Source</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="14"
        height="14"
