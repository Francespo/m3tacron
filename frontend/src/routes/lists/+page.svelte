...
<div class="flex min-h-screen">
    ...
    <main class="flex-1 p-6 md:p-8 pb-20 lg:pb-8">
        ...
        <div class="mb-6"><LocalFilterBar id="lists-local" label="List filters" defaultOpen={true} activeCount={listLocalCount} chips={listLocalChips} onRemoveChip={(k)=>filters.removeChip(k)} onClear={clearListFilters}><div class="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-4 items-start auto-rows-min"><FactionFilter /><StatRangeFilter label="Stat ranges" hideLists showPointsCost /><PilotFilter selectedFactions={filters.selectedFactions} /><ShipChassisFilter selectedFactions={filters.selectedFactions} /></div></LocalFilterBar></div>
