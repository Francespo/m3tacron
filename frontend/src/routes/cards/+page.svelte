<script lang="ts">
  ...
  function syncFromUrl() {
    ...
    const nextTab = params.get("tab") === "upgrades" ? "upgrades" : "pilots";
    ...
    const nextSearch = params.get("search_text") || "";
    if (textSearch !== nextSearch) textSearch = nextSearch;
  ...
  async function loadCards(
    ...
    const resolvedTab =
      url.searchParams.get("tab") === "upgrades" ? "upgrades" : "pilots";
    const endpoint = resolvedTab === "upgrades" ? "upgrades" : "pilots";
  ...
  $effect(() => {
    ...
    loadCards(url, expectedVersion, controller.signal)
      .catch((e) => {
  ...
  $effect(() => {
    ...
    for (const b of filters.selectedBaseSizes) params.append("base_sizes", b);
    ...
    if (tab === "pilots") {
      ...
      if (filters.shieldsMin) params.set("shields_min", filters.shieldsMin);
      if (filters.shieldsMax) params.set("shields_max", filters.shieldsMax);
      if (filters.agilityMin) params.set("agility_min", filters.agilityMin);
      if (filters.agilityMax) params.set("agility_max", filters.agilityMax);
...
{#snippet cardFilters()}
  <div class="space-y-3">
    ...
    {#if isAdvanced}
      <AdvancedFilters isPilotsTab={tab === "pilots"} />
    {/if}
...
<div class="flex min-h-screen">
  ...
  <main class="flex-1 p-6 md:p-8">
    ...
    <div class="flex items-center gap-6 mb-6">
      <button
        class="text-lg font-sans font-bold transition-colors {tab === 'pilots'
          ? 'text-primary'
          : 'text-secondary hover:text-primary'}"
        onclick={() => {
          tab = "pilots";
      ...
      <button
        class="text-lg font-sans font-bold transition-colors {tab === 'upgrades'
          ? 'text-primary'
          : 'text-secondary hover:text-primary'}"
        onclick={() => {
          tab = "upgrades";
    ...
    >
      {#each items as card}
        <a
          href={`/${tab === "pilots" ? "pilot" : "upgrade"}/${card.xws}`}
