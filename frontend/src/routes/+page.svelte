<script lang="ts">
  ...
  let pilotRankingMode = $state<RankingMode>("winrate");
  let upgradeRankingMode = $state<RankingMode>("winrate");
  let shipRankingMode = $state<RankingMode>("winrate");
  let listRankingMode = $state<RankingMode>("winrate");
  ...
  function getUpgradeIconClass(type: string) {
    if (!type) return "";
    return (
      "xwing-miniatures-font-" + type.toLowerCase().replace(/[^a-z0-9]/g, "")
    );
  }
  ...
  function sortByRankingMode(items: any[], rankingMode: RankingMode): any[] {
  ...
  function applyWrMinGames(
    ...
    return items.filter(
      (item) => Number(item?.games_count ?? item?.games ?? 0) >= minGames,
    );
  }
          ...
          datasets: [
            {
              ...
              data: meta.factions.map((d: any) =>
                d.games_count > 0
                  ? Number(((d.wins / d.games_count) * 100).toFixed(1))
                  : 0,
              ),
  ...
  let pieData = $derived(
    meta?.factions
      ? {
          labels: meta.factions.map((d: any) => getFactionLabel(d.xws)),
          datasets: [
            {
              data: meta.factions.map((d: any) => d.games_count),
              backgroundColor: meta.factions.map((d: any) =>
                getFactionColor(d.xws),
              ),
              borderWidth: 0,
...
<div class="min-h-screen p-6 font-sans">
  ...
  {:else}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      ...
      >
        <h2 class="text-sm font-mono font-bold uppercase mb-4 text-primary">
          Faction Performance
        </h2>
      ...
      >
        ...
        <div class="flex flex-wrap justify-center w-full mt-2">
          {#each meta.factions || [] as dist}
            ...
            {@const pct =
              totalFactionGames > 0
                ? (((dist.games_count || 0) / totalFactionGames) * 100).toFixed(
                    1,
                  )
                : "0.0"}
            <div
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      ...
      >
        <div class="mb-4 flex items-center justify-between gap-2">
          ...
          <div
            class="inline-flex rounded border border-border-dark overflow-hidden"
          >
            <button
              type="button"
              class="px-3 py-2 min-h-10 text-[10px] font-mono uppercase tracking-wider transition-colors {pilotRankingMode ===
              'popularity'
            ...
            <button
              type="button"
              class="px-3 py-2 min-h-10 text-[10px] font-mono uppercase tracking-wider border-l border-border-dark transition-colors {pilotRankingMode ===
              'winrate'
                ? 'bg-secondary text-black'
                : 'bg-transparent text-secondary hover:bg-white/5'}"
          ...
          {#each sortedPilots.slice(0, 5) as pilot}
            ...
            >
              ...
              >
                ...
                >
                <span class="text-[11px] text-secondary shrink-0"
      ...
      >
        <div class="mb-4 flex items-center justify-between gap-2">
          ...
          >
            <button
              type="button"
              class="px-3 py-2 min-h-10 text-[10px] font-mono uppercase tracking-wider transition-colors {upgradeRankingMode ===
              'popularity'
                ? 'bg-secondary text-black'
                : 'bg-transparent text-secondary hover:bg-white/5'}"
            ...
            <button
              type="button"
              class="px-3 py-2 min-h-10 text-[10px] font-mono uppercase tracking-wider border-l border-border-dark transition-colors {upgradeRankingMode ===
              'winrate'
                ? 'bg-secondary text-black'
                : 'bg-transparent text-secondary hover:bg-white/5'}"
      ...
      >
        <div class="mb-4 flex items-center justify-between gap-2">
          ...
          >
            <button
              type="button"
              class="px-3 py-2 min-h-10 text-[10px] font-mono uppercase tracking-wider transition-colors {shipRankingMode ===
              'popularity'
                ? 'bg-secondary text-black'
                : 'bg-transparent text-secondary hover:bg-white/5'}"
            ...
            <button
              type="button"
              class="px-3 py-2 min-h-10 text-[10px] font-mono uppercase tracking-wider border-l border-border-dark transition-colors {shipRankingMode ===
              'winrate'
                ? 'bg-secondary text-black'
                : 'bg-transparent text-secondary hover:bg-white/5'}"
      ...
      >
        <div class="mb-4 flex items-center justify-between gap-2">
          ...
          >
            <button
              type="button"
              class="px-3 py-2 min-h-10 text-[10px] font-mono uppercase tracking-wider transition-colors {listRankingMode ===
              'popularity'
                ? 'bg-secondary text-black'
                : 'bg-transparent text-secondary hover:bg-white/5'}"
            ...
            <button
              type="button"
              class="px-3 py-2 min-h-10 text-[10px] font-mono uppercase tracking-wider border-l border-border-dark transition-colors {listRankingMode ===
              'winrate'
                ? 'bg-secondary text-black'
                : 'bg-transparent text-secondary hover:bg-white/5'}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 w-full">
          {#each sortedLists.slice(0, 4) as list}
            ...
            >
              ...
              >
                <div class="flex items-center gap-2 overflow-hidden mr-2 h-12">
                  <i
                    class="xwing-miniatures-font {getFactionIconClass(
                      factionXws,
                      ...
                      {#if pilot.upgrades?.length}
                        <div class="text-[11px] text-secondary/80 truncate">
                          {pilot.upgrades
                            ...
                            .map(
                              (u: any) =>
                                xwingData.getUpgrade(u.xws)?.name || u.xws,
                            )
                            .join(", ")}
                        </div>
                      {/if}
