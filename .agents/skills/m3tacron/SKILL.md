...
### Connection command
```bash
ssh -i ~/.ssh/m3tacron_audit_bot -o BatchMode=yes -o StrictHostKeyChecking=accept-new audit-bot@84.8.253.2
```
...
- DB credentials are runtime secrets. Retrieve them from the authorized server environment when needed; never store or print them in repository files, logs, issues, or pull requests.

### Running database queries
Use read-only SQL by default and target the database container discovered at runtime:
```bash
docker exec <db-container> psql -U postgres -v ON_ERROR_STOP=1 -c "SELECT COUNT(*) FROM tournament;"
```
Do not print environment variables or connection strings. Any write query requires explicit human authorization.
